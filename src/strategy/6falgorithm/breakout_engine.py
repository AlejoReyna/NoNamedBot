...
class BreakoutEngine:
    ...
    def evaluate_token(
        ...
        if self.sentiment_tier1 is not None:
            try:
                ...
                token_sentiment = {}
        entry_score = self._entry_score(candidate, momentum_z_score=0.0, token_sentiment=token_sentiment, atr_ratio=candidate.atr_ratio)
    ...
    def _entry_score(self, candidate: _CheapCandidate, momentum_z_score: float, token_sentiment: dict[str, Any] | None = None, atr_ratio: float = 1.0) -> float:
        ...
        score += weights["derivatives"] * (1.0 if candidate.derivatives_risk_clear else 0.0)
